...
func RequestDeviceSession(serial, proof []byte, oldSession string) (string, error) {
	...
	// TODO: this should change server-side to expect serial + proof ^
	...
	req.Header.Set("Content-Type", "application/json")
	if oldSession != "" {
		req.Header.Set("X-Device-Authorization", fmt.Sprintf(`Macaroon root="%s"`, oldSession))
        }
